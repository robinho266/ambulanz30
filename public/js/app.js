...
$(function () {
	var $humanBody = $('#humanBody');

	if ($humanBody.length > 0) {
		var $input = $('#localisation');
		var localisation = $input.val() ? $input.val().split(',') : [];

		localisation.map(function (item) {
		...
		$humanBody.on('click', 'path', function () {
			if ($(this).parents('.mutable').length > 0) {
				...
				var index = localisation.indexOf(part);
